...
class Customer:
    ...
    def calculate_distance(self, shop: Shop) -> float:
        x1, y1 = self.location
        return (math.sqrt(
            (shop.location[0] - x1) ** 2 +
            (shop.location[1] - y1) ** 2))

    def calculate_trip_cost(self, shops: list[Shop], fuel_price: float) -> tuple:
        trip_costs = dict()
        for shop in shops:
            if all(product in shop.products for product in self.product_cart.keys()):
                ...
                trip_costs[shop] = (fuel_price * self.car.fuel_consumption
                                    * self.calculate_distance(shop) +
                                    products_price)
                print(f"{self.name}'s trip to the {shop.name} costs {trip_costs[shop]}")
            else:
                raise ValueError
