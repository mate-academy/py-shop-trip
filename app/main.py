...
def shop_trip() -> None:
    ...
    for customer in customers:
        ...
        for shop in shops:
            distance_to_shop = Customer.calculate_distance(
                customer.location,
                shop.location
            )
            fuel_cost_to_shop = distance_to_shop / 100
            fuel_cost_to_shop *= customer.car.fuel_consumption
            fuel_cost_to_shop *= fuel_price

            product_cost = sum(
                customer.product_cart[item]
                * shop.products[item] for item in customer.product_cart
            )
            fuel_cost_to_home = distance_to_shop / 100
            fuel_cost_to_home *= customer.car.fuel_consumption
            fuel_cost_to_home *= fuel_price
            total_trip_cost = fuel_cost_to_shop
            total_trip_cost += product_cost
            total_trip_cost += fuel_cost_to_home
            ...
            if (
                    total_trip_cost < min_trip_cost
            ) and (
                    total_trip_cost <= customer.money
            ):
        ...
        if selected_shop:
            ...
            customer.purchase_receipt(
                selected_shop.name, selected_shop.products
            )
