import json
from car import Car
from shop import Shop, shop_list
...
class Customer:
    ...
    def select_shop(self) -> tuple[Shop, int]:
        ...
        for shop in shop_list:
            total_expenses = (self.calculate_fuel_cost(shop.location)
                              + self.calculate_products_cost(shop.products))
    ...
    def go_to_shop(self, shop: Shop) -> None:
        ...
        print(f"Date: {datetime.now().strftime('%x %X')}")
