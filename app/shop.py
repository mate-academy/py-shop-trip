...
class Shop:
    ...
    def check(self, customer: Customer) -> None:
        ...
        print(f"Total cost is {self.check_price} dollars\n"
              f"See you again!\n")

    def trip_price_counting(self, customer: Customer, fuel_price: float) -> None:
        distance = sqrt((customer.location[0] - self.location[0]) ** 2 + (customer.location[1] - self.location[1]) ** 2)
        fuel = (distance * customer.car.fuel_consumption) / 100 * fuel_price
        full_price = fuel * 2
        for product, price in self.products.items():
            if product in customer.product_cart:
                full_price += customer.product_cart[product] * price
        print(f"{customer.name}'s trip to the {self.name} costs {round(full_price, 2)}")
